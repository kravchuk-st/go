...
function toogleVideoStatus() {
  if (video.paused) {
    ...
    play.setAttribute("xlink:href", "./assets/img/svg/player/pause.svg#pause-btn");
  } else {
    video.pause();
    play.setAttribute("xlink:href", "./assets/img/svg/player/play.svg#play-btn");
  };
}
...
function updateMute() {
  if (previusVolume !== null) {
    ...
    previusVolume = null;
    mute.setAttribute("xlink:href", "./assets/img/svg/player/volume.svg#volume-btn");
  } else {
    previusVolume = video.volume;
    video.volume = 0;
    mute.setAttribute("xlink:href", "./assets/img/svg/player/mute.svg#mute-btn");
  }
}
...
function updateProgressCSS(){
  ...
  progress.style.background = `linear-gradient(to right, #fccb1f 0%, #fccb1f ${Math.ceil(prog * 10 + 1) / 10}%, #fff ${Math.ceil(prog * 10 + 1) / 10}%, white 100%)`;
}
